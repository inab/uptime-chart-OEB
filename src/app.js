				...
				for (i = 0; i < d.length; i++) {
					...
					if (name == 'Access Time') {
						...
						if (value == 200) {
							...
							text +=
								"<td class='value'> No information captured </td>";
						} else {
...
async function loadChart(elems) {
	...
	for (let y of elems) {
		try {
			...
			y.appendChild(div);
			genChartData(
				await fetchUrl(chartUrl, limit, mode),
				divid,
				xaxis,
				c_w,
				c_h,
...
async function fetchUrl(url, limit, mode) {
	try {
		...
		let startDate = currentDate - limit * 86400;

		let request = await fetch(
			'https://' +
				base_url +
				'.bsc.es/monitor/rest/homepage/' +
				url +
				'?date1=' +
				startDate +
				'&date2=' +
